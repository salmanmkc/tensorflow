...
absl::Status GpuCompiler::AddConvAndGemmAutotuningPasses(
  ...
  auto add_backend = [&](std::string name, auto creator) {
    if (enabled_backends.empty() ||
        absl::c_linear_search(enabled_backends, name)) {
      backends.push_back(creator());
    }
  };

  add_backend("Cudnn", [&] {
    return std::make_unique<CudnnBackend>(stream_exec, &debug_options, this,
                                          target_config);
  });
  add_backend("Triton", [&] {
    return std::make_unique<TritonBackend>(&debug_options, this, target_config,
                                           mlir_context);
  });
  add_backend("Cublas", [&] {
    return std::make_unique<CublasBackend>(stream_exec, &debug_options, this,
                                           target_config);
  });
  add_backend("CublasLt", [&] {
    return std::make_unique<CublasLtBackend>(stream_exec, &debug_options, this,
                                             target_config);
  });
  add_backend("Cublas_fission", [&] {
    return std::make_unique<FissionBackend>(
  ...
  add_backend("CustomKernel_fission", [&] {
    return std::make_unique<FissionBackend>(
        &debug_options, this, target_config,
  ...
  if (!debug_options.xla_gpu_experimental_disable_binary_libraries()) {
    TF_ASSIGN_OR_RETURN(
        ...
        GetCodegenBackends(stream_exec, target_config, debug_options));
    for (auto& backend : platform_backends) {
      if (enabled_backends.empty() ||
          absl::c_linear_search(enabled_backends, backend->name())) {
