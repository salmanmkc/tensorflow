...
std::string TransposePlan::Loop::ToString() const {
  return absl::StrFormat("%d%s[ds=%d,ts=%d,%d,%d](lda=%d,ldb=%d,p=%d,c=%d)",
                         dim_in_a, tile_interior ? "[tile]" : "", dim_size,
                         tile_size, start, end, lda, ldb, parallelism,
                         is_contiguous);
}
...
void TransposePlan::Initialize() {
  ...
  auto loops_to_string = [](absl::Span<const Loop> loops) {
    return absl::StrJoin(loops, ", ", [](std::string* out, const Loop& l) {
...
void TransposePlan::IdentifyContiguousLoops(
  ...
  if (chunk_contiguity_ != ChunkContiguity::kNone) {
    int64_t target_chunk_elems =
        CeilOfRatio<int64_t>(num_elems_, num_chunks_requested_);
    ...
    absl::c_stable_sort(indices, [&](size_t a, size_t b) {
      return std::make_tuple(!loop_order[a].tile_interior,
                             contiguous_stride(loop_order[a])) <
             std::make_tuple(!loop_order[b].tile_interior,
                             contiguous_stride(loop_order[b]));
    });

    // Accumulate loops with smallest strides until we reach target size.
    int64_t accumulated_elems = 1;
    for (size_t idx : indices) {
      Loop& loop = loop_order[idx];
      if (contiguous_stride(loop) == 0) {
        continue;
      }
      int64_t loop_elems = loop.tile_interior
                               ? loop.tile_size
                               : CeilOfRatio(loop.dim_size, loop.tile_size);
      accumulated_elems *= loop_elems;
      loop.is_contiguous = true;
      if (accumulated_elems >= target_chunk_elems) {
        break;
      }
    }
  }
...
void TransposePlan::ChooseLoopOrder(std::vector<Loop>& loop_order) const {
  ...
  auto cost = [&](const Loop& l) {
    ...
    // Contiguous loops must sort last (innermost) to maintain chunk contiguity.
    return std::make_tuple(
        dim_must_go_last, inner_kernel_is_memcpy_ && l.tile_interior,
        l.is_contiguous, -std::min<double>(a_stride * penalty, b_stride));
  };
