...
TEST_F(HloShardingSpecTest, IsFullyReplicated) {
  int num_shards = 6;
  {
    ...
    std::shared_ptr<const HloShardingSpec> spec =
        HloShardingSpec::Create(num_shards, MemoryKind(), xla_hlo_sharding);
...
TEST_F(HloShardingSpecTest, HasSamePartitioning) {
  ...
  std::shared_ptr<const HloShardingSpec> spec0 =
      HloShardingSpec::Create(num_shards0, MemoryKind(), xla_hlo_sharding0);
  ...
  EXPECT_TRUE(spec0->HasSamePartitioning(*spec0));
  {
    ...
    std::shared_ptr<const HloShardingSpec> spec1 =
        HloShardingSpec::Create(num_shards1, MemoryKind(), xla_hlo_sharding1);
  ...
  // Different HloSharding.
  {
    ...
    std::shared_ptr<const HloShardingSpec> spec1 =
        HloShardingSpec::Create(num_shards0, MemoryKind(), xla_hlo_sharding1);
    EXPECT_FALSE(spec0->HasSamePartitioning(*spec1));
  }
  ...
  {
    int num_shards1 = 3;
    std::shared_ptr<const HloShardingSpec> spec0 = HloShardingSpec::Create(
        num_shards0, MemoryKind(), xla::HloSharding::Replicate());
    std::shared_ptr<const HloShardingSpec> spec1 = HloShardingSpec::Create(
        num_shards1, MemoryKind(), xla::HloSharding::Replicate());
    EXPECT_FALSE(spec0->HasSamePartitioning(*spec1));
  }
}
...
TEST_F(HloShardingSpecTest, DisassembleFailsWithInvalidDeviceCount) {
  int num_shards = 1;
...
TEST_F(HloShardingSpecTest, Hash) {
  EXPECT_TRUE(absl::VerifyTypeImplementsAbslHashCorrectly({
      HloShardingSpec::Create(6, MemoryKind(), xla::HloSharding::Replicate()),
      HloShardingSpec::Create(1, MemoryKind(), xla::HloSharding::Replicate()),
      HloShardingSpec::Create(1, MemoryKind("pinned_host"),
                              xla::HloSharding::Replicate()),
      HloShardingSpec::Create(6, MemoryKind(),
