...
std::unique_ptr<HloShardingSpec> HloShardingSpec::Create(
    int num_shards, MemoryKind memory_kind, xla::HloSharding xla_hlo_sharding) {
  memory_kind = CanonicalizeMemoryKind(memory_kind);
  return std::unique_ptr<HloShardingSpec>(new HloShardingSpec(
      num_shards, memory_kind, std::move(xla_hlo_sharding)));
}

HloShardingSpec::HloShardingSpec(int num_shards, MemoryKind memory_kind,
                                 xla::HloSharding xla_hlo_sharding)
    : llvm::RTTIExtends<HloShardingSpec, XlaCompatibleShardingSpec>(
          num_shards, memory_kind,
...
absl::StatusOr<std::unique_ptr<ShardingSpec>> HloShardingSpec::WithMemoryKind(
    MemoryKind memory_kind) const {
  return Create(num_shards_, memory_kind, xla_hlo_sharding_);
}
...
HloShardingSpec::Disassemble(const Shape& shape) const {
  ...
  if (is_even_sharding) {
    ...
    for (int i = 0; i < num_shards_; ++i) {
      result.push_back({
          shard_shape,
          SingleDeviceShardingSpec::Create(memory_kind_),
      });
    }
    return result;
  }
...
std::string HloShardingSpec::DebugString() const {
  return absl::StrFormat(
      "HloShardingSpec(num_shards: %d, memory_kind: %v, hlo_sharding: %s)",
      num_shards_, memory_kind_, xla_hlo_sharding_.ToString());
}

void HloShardingSpec::Hash(absl::HashState state) const {
  uint64_t hash = hash_.load(std::memory_order_relaxed);
  if (hash == kUnsetHash) {
    hash = absl::HashOf(num_shards_, memory_kind_, xla_hlo_sharding_);
