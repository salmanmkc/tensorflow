...
TEST_P(ShardingSpecSerDesTest, SingleDeviceShardingSpecRoundTrip) {
  auto sharding_spec = SingleDeviceShardingSpec::Create(MemoryKind("abc"));
  ...
  EXPECT_THAT(out_sharding_spec->memory_kind(), sharding_spec->memory_kind());
}

TEST_P(ShardingSpecSerDesTest, OpaqueShardingSpecRoundTrip) {
  auto sharding_spec =
      OpaqueShardingSpec::Create(num_shards(), MemoryKind("abc"));
...
TEST_P(ShardingSpecSerDesTest, ConcreteShardingSpecRoundTrip) {
  ...
  auto sharding_spec =
      ConcreteShardingSpec::Create(num_shards(), MemoryKind("abc"),
