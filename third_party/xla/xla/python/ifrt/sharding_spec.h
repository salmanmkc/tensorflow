...
class ShardingSpec : public llvm::RTTIExtends<ShardingSpec, Serializable> {
 public:
  ...
  // Returns the memory kind for all shards in this sharding spec.
  ...
  // Returns a new sharding spec with the same logical partitioning as this
  // sharding spec, but with a different memory kind. `memory_kind` must be a
  // valid memory kind for the devices used.
  virtual absl::StatusOr<std::unique_ptr<ShardingSpec>> WithMemoryKind(
      MemoryKind memory_kind) const = 0;
 ...
 protected:
  ShardingSpec(int num_shards, MemoryKind memory_kind,
...
class SingleDeviceShardingSpec final
 ...
 public:
  ...
  absl::StatusOr<std::unique_ptr<ShardingSpec>> WithMemoryKind(
 ...
 private:
  explicit SingleDeviceShardingSpec(MemoryKind memory_kind);
...
class OpaqueShardingSpec
 ...
 private:
  explicit OpaqueShardingSpec(int num_shards, MemoryKind memory_kind);
...
class ConcreteShardingSpec
 ...
 public:
  ...
  // shapes.
  // REQUIRES: num_shards == shard_shapes.size()
 ...
 private:
  ...
  ConcreteShardingSpec(int num_shards, MemoryKind memory_kind,
...
class ConcreteEvenShardingSpec
 ...
 private:
  ConcreteEvenShardingSpec(int num_shards, MemoryKind memory_kind, Shape shape,
...
class ShardingParamShardingSpec
 ...
 public:
  static absl::StatusOr<std::unique_ptr<ShardingParamShardingSpec>> Create(
 ...
 private:
  ShardingParamShardingSpec(int num_shards, MemoryKind memory_kind,
