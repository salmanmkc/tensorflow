...
// Returns a canonicalized memory kind.
...
ShardingSpec::ShardingSpec(int num_shards, MemoryKind memory_kind,
...
std::unique_ptr<SingleDeviceShardingSpec> SingleDeviceShardingSpec::Create(
  ...
  return std::unique_ptr<SingleDeviceShardingSpec>(
      new SingleDeviceShardingSpec(CanonicalizeMemoryKind(memory_kind)));
}

SingleDeviceShardingSpec::SingleDeviceShardingSpec(MemoryKind memory_kind)
    : llvm::RTTIExtends<SingleDeviceShardingSpec, ShardingSpec>(
          /*num_shards=*/1, memory_kind,
...
absl::StatusOr<std::unique_ptr<ShardingSpec>>
SingleDeviceShardingSpec::WithMemoryKind(MemoryKind memory_kind) const {
  return Create(memory_kind);
}
...
SingleDeviceShardingSpec::Disassemble(const Shape& shape) const {
  return std::vector<std::pair<Shape, ShardingSpecRef>>{
      {shape, SingleDeviceShardingSpec::Create(memory_kind_)}};
}
...
SingleDeviceShardingSpec::Disassemble(const DynamicShape& dynamic_shape) const {
  return std::vector<std::pair<DynamicShape, ShardingSpecRef>>{
      {dynamic_shape, SingleDeviceShardingSpec::Create(memory_kind_)}};
}
...
std::string SingleDeviceShardingSpec::DebugString() const {
  return absl::StrFormat("SingleDeviceShardingSpec(memory_kind: %v)",
                         memory_kind_);
}

void SingleDeviceShardingSpec::Hash(absl::HashState state) const {
  absl::HashState::combine(std::move(state), memory_kind_);
}

std::unique_ptr<OpaqueShardingSpec> OpaqueShardingSpec::Create(
    int num_shards, MemoryKind memory_kind) {
  return std::unique_ptr<OpaqueShardingSpec>(
      new OpaqueShardingSpec(num_shards, CanonicalizeMemoryKind(memory_kind)));
}

OpaqueShardingSpec::OpaqueShardingSpec(int num_shards, MemoryKind memory_kind)
    : llvm::RTTIExtends<OpaqueShardingSpec, ShardingSpec>(
          num_shards, memory_kind, /*is_fully_replicated=*/false) {}
...
absl::StatusOr<std::unique_ptr<ShardingSpec>>
OpaqueShardingSpec::WithMemoryKind(MemoryKind memory_kind) const {
  return Create(num_shards_, memory_kind);
}
...
std::string OpaqueShardingSpec::DebugString() const {
  return absl::StrFormat("OpaqueShardingSpec(num_shards: %d, memory_kind: %v)",
                         num_shards_, memory_kind_);
}

void OpaqueShardingSpec::Hash(absl::HashState state) const {
  absl::HashState::combine(std::move(state), num_shards_, memory_kind_);
}

std::unique_ptr<ConcreteShardingSpec> ConcreteShardingSpec::Create(
    int num_shards, MemoryKind memory_kind, Shape shape,
  ...
  return std::unique_ptr<ConcreteShardingSpec>(new ConcreteShardingSpec(
      num_shards, CanonicalizeMemoryKind(memory_kind), std::move(shape),
      std::move(shard_shapes), std::move(index_domains)));
}

std::unique_ptr<ConcreteShardingSpec> ConcreteShardingSpec::Create(
    int num_shards, MemoryKind memory_kind, DynamicShape dynamic_shape,
    std::vector<DynamicShape> shard_dynamic_shapes) {
  return std::unique_ptr<ConcreteShardingSpec>(new ConcreteShardingSpec(
      num_shards, CanonicalizeMemoryKind(memory_kind), std::move(dynamic_shape),
...
absl::StatusOr<std::unique_ptr<ShardingSpec>>
ConcreteShardingSpec::WithMemoryKind(MemoryKind memory_kind) const {
  if (has_static_shape()) {
    return Create(num_shards_, memory_kind, std::get<Shape>(shape_),
                  std::get<std::vector<Shape>>(shard_shapes_), index_domains_);
  }
  return Create(num_shards_, memory_kind, std::get<DynamicShape>(shape_),
...
ConcreteShardingSpec::Disassemble(const Shape& shape) const {
  ...
  for (const auto& shard_shape : shard_shapes) {
    result.push_back(
        {shard_shape, SingleDeviceShardingSpec::Create(memory_kind_)});
  }
  return result;
}
...
ConcreteShardingSpec::Disassemble(const DynamicShape& dynamic_shape) const {
  ...
  for (const auto& shard_dynamic_shape : shard_dynamic_shapes) {
    result.push_back(
        {shard_dynamic_shape, SingleDeviceShardingSpec::Create(memory_kind_)});
  }
  return result;
}
  ...
  return std::visit(
      [this](const auto& shape, const auto& shard_shapes) {
        return absl::StrFormat(
            "ConcreteShardingSpec(num_shards: %d, memory_kind: %v, shape: %s, "
            "shard_shapes: [%s], index_domains: %s)",
            num_shards_, memory_kind_, shape.DebugString(),
...
void ConcreteShardingSpec::Hash(absl::HashState state) const {
  absl::HashState::combine(std::move(state), num_shards_, memory_kind_, shape_,
                           shard_shapes_);
}

std::unique_ptr<ConcreteEvenShardingSpec> ConcreteEvenShardingSpec::Create(
    int num_shards, MemoryKind memory_kind, Shape shape, Shape shard_shape,
    bool is_fully_replicated) {
  return std::unique_ptr<ConcreteEvenShardingSpec>(new ConcreteEvenShardingSpec(
      num_shards, CanonicalizeMemoryKind(memory_kind), std::move(shape),
      std::move(shard_shape), is_fully_replicated));
}

ConcreteEvenShardingSpec::ConcreteEvenShardingSpec(int num_shards,
                                                   MemoryKind memory_kind,
                                                   Shape shape,
                                                   Shape shard_shape,
                                                   bool is_fully_replicated)
    : llvm::RTTIExtends<ConcreteEvenShardingSpec, ShardingSpec>(
          num_shards, memory_kind, is_fully_replicated),
      shape_(std::move(shape)),
...
absl::StatusOr<std::unique_ptr<ShardingSpec>>
ConcreteEvenShardingSpec::WithMemoryKind(MemoryKind memory_kind) const {
  return Create(num_shards_, memory_kind, shape_, shard_shape_,
                is_fully_replicated_);
}
...
ConcreteEvenShardingSpec::Disassemble(const Shape& shape) const {
  ...
  for (int i = 0; i < num_shards_; ++i) {
    result.push_back(
        {shard_shape_, SingleDeviceShardingSpec::Create(memory_kind_)});
  }
  return result;
}
...
std::string ConcreteEvenShardingSpec::DebugString() const {
  return absl::StrFormat(
      "ConcreteEvenShardingSpec(num_shards: %d, memory_kind: %v, shape: %s, "
      "shard_shape: %s, is_fully_replicated: %s)",
      num_shards_, memory_kind_, shape_.DebugString(),
      shard_shape_.DebugString(), is_fully_replicated_ ? "true" : "false");
}
...
absl::StatusOr<std::unique_ptr<ShardingParamShardingSpec>>
ShardingParamShardingSpec::Create(int num_shards, MemoryKind memory_kind,
                                  ShardingParam sharding_param) {
  int64_t param_device_count =
      absl::c_accumulate(sharding_param.minor_to_major().axis_sizes, 1,
                         std::multiplies<int64_t>());
  if (param_device_count != num_shards) {
    return InvalidArgument(
        "Shard counts don't match. From ShardingParam %d vs from num_shards %d",
        param_device_count, num_shards);
  }
  return std::unique_ptr<ShardingParamShardingSpec>(
      new ShardingParamShardingSpec(num_shards,
                                    CanonicalizeMemoryKind(memory_kind),
                                    std::move(sharding_param)));
}

ShardingParamShardingSpec::ShardingParamShardingSpec(
    int num_shards, MemoryKind memory_kind, ShardingParam sharding_param)
    : llvm::RTTIExtends<ShardingParamShardingSpec, ShardingSpec>(
          num_shards, memory_kind, ComputeIsFullyReplicated(sharding_param)),
...
absl::StatusOr<std::unique_ptr<ShardingSpec>>
ShardingParamShardingSpec::WithMemoryKind(MemoryKind memory_kind) const {
  return Create(num_shards_, memory_kind, sharding_param_);
}
...
ShardingParamShardingSpec::Disassemble(const Shape& shape) const {
  ...
  for (int i = 0; i < num_shards_; ++i) {
    result.push_back(
        {local_shape, SingleDeviceShardingSpec::Create(memory_kind_)});
  }
  return result;
}
...
std::string ShardingParamShardingSpec::DebugString() const {
  return absl::StrFormat(
      "ShardingParamShardingSpec(num_shards: %d, memory_kind: %v, %s)",
      num_shards_, memory_kind_, sharding_param_.DebugString());
}
