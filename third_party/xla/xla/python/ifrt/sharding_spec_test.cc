...
TEST_P(SingleDeviceShardingSpecTest, IsFullyReplicated) {
  ShardingSpecRef sharding = SingleDeviceShardingSpec::Create(MemoryKind());
...
TEST_P(SingleDeviceShardingSpecTest, HasSamePartitioning) {
  ShardingSpecRef sharding0 = SingleDeviceShardingSpec::Create(MemoryKind());

  EXPECT_TRUE(sharding0->HasSamePartitioning(*sharding0));
  {
    ShardingSpecRef sharding1 = SingleDeviceShardingSpec::Create(MemoryKind());
    EXPECT_TRUE(sharding0->HasSamePartitioning(*sharding1));
  }
}

TEST_P(SingleDeviceShardingSpecTest, WithMemoryKind) {
  ShardingSpecRef sharding0 =
      SingleDeviceShardingSpec::Create(MemoryKind("pinned_host"));
  {
    ShardingSpecRef sharding1 =
        SingleDeviceShardingSpec::Create(MemoryKind("device"));
    TF_ASSERT_OK_AND_ASSIGN(auto new_sharding,
                            sharding0->WithMemoryKind(MemoryKind("device")));
...
TEST_P(SingleDeviceShardingSpecTest, Hash) {
...
TEST_P(OpaqueShardingSpecTest, IsFullyReplicated) {
  ShardingSpecRef sharding = OpaqueShardingSpec::Create(2, MemoryKind());
  EXPECT_FALSE(sharding->IsFullyReplicated());
}

TEST_P(OpaqueShardingSpecTest, GetShardShape) {
  ShardingSpecRef sharding =
      OpaqueShardingSpec::Create(num_shards(), MemoryKind());
  EXPECT_THAT(
...
TEST_P(OpaqueShardingSpecTest, HasSamePartitioning) {
  ShardingSpecRef sharding0 = OpaqueShardingSpec::Create(2, MemoryKind());

  EXPECT_TRUE(sharding0->HasSamePartitioning(*sharding0));
  // OpaqueShardingSpec::HasSamePartitioning currently only returns true for the
  // same object.
  ShardingSpecRef sharding1 = OpaqueShardingSpec::Create(2, MemoryKind());
  EXPECT_FALSE(sharding0->HasSamePartitioning(*sharding1));
}

TEST_P(OpaqueShardingSpecTest, WithMemoryKind) {
  ShardingSpecRef sharding0 =
      OpaqueShardingSpec::Create(num_shards(), MemoryKind("pinned_host"));
  {
    TF_ASSERT_OK_AND_ASSIGN(auto new_sharding,
                            sharding0->WithMemoryKind(MemoryKind("device")));
    // For OpaqueShardingSpec, we cannot use an equality test as
    // HasSamePartitioning returns false for different objects.
    ASSERT_TRUE(llvm::isa<OpaqueShardingSpec>(*new_sharding));
    EXPECT_EQ(new_sharding->num_shards(), num_shards());
    EXPECT_EQ(new_sharding->memory_kind(), MemoryKind("device"));
  }
}
...
TEST_P(OpaqueShardingSpecTest, Hash) {
  EXPECT_TRUE(absl::VerifyTypeImplementsAbslHashCorrectly({
      *OpaqueShardingSpec::Create(2, MemoryKind("pinned_host")),
      *OpaqueShardingSpec::Create(2, MemoryKind("device")),
      *OpaqueShardingSpec::Create(3, MemoryKind("pinned_host")),
  }));
}

TEST_P(ConcreteShardingSpecTest, IsFullyReplicated) {
  std::vector<Shape> shard_shapes{Shape({10}), Shape({20})};
  ShardingSpecRef sharding = ConcreteShardingSpec::Create(
      /*num_shards=*/2, MemoryKind(), Shape({30}), shard_shapes);
  EXPECT_FALSE(sharding->IsFullyReplicated());
}
...
TEST_P(ConcreteShardingSpecTest, HasSamePartitioning) {
  ...
  ShardingSpecRef sharding0 = ConcreteShardingSpec::Create(
      /*num_shards=*/2, MemoryKind(), Shape({30}), shard_shapes0);

  EXPECT_TRUE(sharding0->HasSamePartitioning(*sharding0));
  {
    std::vector<Shape> shard_shapes1{Shape({10}), Shape({20})};
    ShardingSpecRef sharding1 = ConcreteShardingSpec::Create(
        /*num_shards=*/2, MemoryKind(), Shape({30}), shard_shapes1);
    EXPECT_TRUE(sharding0->HasSamePartitioning(*sharding1));
  }
  // Different number of shards.
  {
    ...
    ShardingSpecRef sharding1 = ConcreteShardingSpec::Create(
        /*num_shards=*/3, MemoryKind(), Shape({60}), shard_shapes1);
    EXPECT_FALSE(sharding0->HasSamePartitioning(*sharding1));
  }
  // Difference shape.
  {
    std::vector<Shape> shard_shapes1{Shape({10}), Shape({20})};
    ShardingSpecRef sharding1 = ConcreteShardingSpec::Create(
        /*num_shards=*/2, MemoryKind(), Shape({40}), shard_shapes1);
    EXPECT_FALSE(sharding0->HasSamePartitioning(*sharding1));
  }
...
TEST_P(ConcreteShardingSpecTest, WithMemoryKind) {
  std::vector<Shape> shard_shapes0{Shape({10}), Shape({20})};
  ShardingSpecRef sharding0 = ConcreteShardingSpec::Create(
      /*num_shards=*/2, MemoryKind("pinned_host"), Shape({30}), shard_shapes0);
  {
    TF_ASSERT_OK_AND_ASSIGN(auto new_sharding,
                            sharding0->WithMemoryKind(MemoryKind("device")));
    std::vector<Shape> shard_shapes1{Shape({10}), Shape({20})};
    ShardingSpecRef sharding1 = ConcreteShardingSpec::Create(
        /*num_shards=*/2, MemoryKind("device"), Shape({30}), shard_shapes1);
    EXPECT_EQ(*new_sharding, *sharding1);
  }
}
...
TEST_P(ConcreteShardingSpecTest, Disassemble) {
  std::vector<Shape> shard_shapes{Shape({3}), Shape({7}), Shape({3}),
                                  Shape({7})};
  ShardingSpecRef sharding = ConcreteShardingSpec::Create(
      /*num_shards=*/4, MemoryKind(), Shape({20}), shard_shapes);
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    EXPECT_EQ(*result_sharding,
              *SingleDeviceShardingSpec::Create(MemoryKind()));
  }
}
...
TEST_P(ConcreteShardingSpecTest, DisassembleDynamicShape) {
  ...
  auto sharding = ConcreteShardingSpec::Create(
      /*num_shards=*/4, MemoryKind(), dynamic_shape, shard_dynamic_shapes);
...
TEST_P(ConcreteShardingSpecTest, IndexDomains) {
  ...
  ShardingSpecRef sharding = ConcreteShardingSpec::Create(
      /*num_shards=*/2, MemoryKind(), Shape({15}), shard_shapes, index_domains);
...
TEST_P(ConcreteShardingSpecTest, Hash) {
  TF_ASSERT_OK_AND_ASSIGN(
      ...
      DynamicShape::Create(Shape({30}), BoundedDynamicShapeTag({true})));
  EXPECT_TRUE(absl::VerifyTypeImplementsAbslHashCorrectly({
      *ConcreteShardingSpec::Create(/*num_shards=*/2, MemoryKind(), Shape({30}),
                                    {Shape({10}), Shape({20})}),
      *ConcreteShardingSpec::Create(/*num_shards=*/2, MemoryKind(),
                                    dynamic_shape,
...
TEST_P(ConcreteEvenShardingSpecTest, IsFullyReplicated) {
  {
    ...
    ShardingSpecRef sharding = ConcreteEvenShardingSpec::Create(
        /*num_shards=*/2, MemoryKind(), Shape({30}), Shape({15}),
...
TEST_P(ConcreteEvenShardingSpecTest, HasSamePartitioning) {
  ...
  {
    ShardingSpecRef sharding1 = ConcreteEvenShardingSpec::Create(
        /*num_shards=*/3, MemoryKind(), Shape({30}), Shape({15}),
        /*is_fully_replicated=*/true);
    EXPECT_FALSE(sharding0->HasSamePartitioning(*sharding1));
  }
  // Difference shape.
  {
    ShardingSpecRef sharding1 = ConcreteEvenShardingSpec::Create(
        /*num_shards=*/2, MemoryKind(), Shape({45}), Shape({15}),
        /*is_fully_replicated=*/true);
    EXPECT_FALSE(sharding0->HasSamePartitioning(*sharding1));
  }
  ...
  {
    ShardingSpecRef sharding1 = ConcreteEvenShardingSpec::Create(
        /*num_shards=*/2, MemoryKind(), Shape({30}), Shape({10}),
        /*is_fully_replicated=*/true);
    EXPECT_FALSE(sharding0->HasSamePartitioning(*sharding1));
  }
...
TEST_P(ConcreteEvenShardingSpecTest, WithMemoryKind) {
  ShardingSpecRef sharding0 = ConcreteEvenShardingSpec::Create(
      /*num_shards=*/2, MemoryKind("pinned_host"), Shape({30}), Shape({15}),
      /*is_fully_replicated=*/true);
  {
    TF_ASSERT_OK_AND_ASSIGN(auto new_sharding,
                            sharding0->WithMemoryKind(MemoryKind("device")));
    ShardingSpecRef sharding1 = ConcreteEvenShardingSpec::Create(
        /*num_shards=*/2, MemoryKind("device"), Shape({30}), Shape({15}),
        /*is_fully_replicated=*/true);
    EXPECT_EQ(*new_sharding, *sharding1);
  }
}
...
TEST_P(ConcreteEvenShardingSpecTest, Disassemble) {
  ShardingSpecRef sharding = ConcreteEvenShardingSpec::Create(
      num_shards(), MemoryKind(), Shape({30}), Shape({5}),
      /*is_fully_replicated=*/false);
...
TEST_P(ConcreteEvenShardingSpecTest, IndexDomainsFails) {
  ShardingSpecRef sharding = ConcreteEvenShardingSpec::Create(
      /*num_shards=*/2, MemoryKind(), Shape({30}), Shape({5}),
      /*is_fully_replicated=*/false);
...
TEST_P(ConcreteEvenShardingSpecTest, Hash) {
  EXPECT_TRUE(absl::VerifyTypeImplementsAbslHashCorrectly({
      *ConcreteEvenShardingSpec::Create(
          /*num_shards=*/2, MemoryKind("pinned_host"), Shape({30}), Shape({30}),
          /*is_fully_replicated=*/true),
      *ConcreteEvenShardingSpec::Create(/*num_shards=*/2, MemoryKind("device"),
                                        Shape({30}), Shape({15}),
                                        /*is_fully_replicated=*/false),
      *ConcreteEvenShardingSpec::Create(
          /*num_shards=*/3, MemoryKind("pinned_host"), Shape({30}), Shape({10}),
          /*is_fully_replicated=*/false),
  }));
}

TEST_P(ShardingParamShardingSpecTest, CreateFailsWhenShardCountNotMatch) {
  ShardingParam param{/*dim_shards=*/{2, 3},
                      {/*permutation=*/{1, 0}, /*axis_sizes=*/{3, 2}}};

  EXPECT_THAT(ShardingParamShardingSpec::Create(2, MemoryKind(), param),
              absl_testing::StatusIs(
                  tsl::error::INVALID_ARGUMENT,
                  HasSubstr("Shard counts don't match. From ShardingParam 6 "
                            "vs from num_shards 2")));
}
...
TEST_P(ShardingParamShardingSpecTest, IsFullyReplicated) {
  {
    ...
    ShardingParam param{/*dim_shards=*/{1, 1},
                        {/*permutation=*/{1, 0}, /*axis_sizes=*/{3, 2}}};
    TF_ASSERT_OK_AND_ASSIGN(
        ShardingSpecRef param_sharding,
        ShardingParamShardingSpec::Create(6, MemoryKind(), param));
...
TEST_P(ShardingParamShardingSpecTest, HasSamePartitioning) {
  ...
  TF_ASSERT_OK_AND_ASSIGN(
      ShardingSpecRef sharding0,
      ShardingParamShardingSpec::Create(6, MemoryKind(), param0));
  ...
  EXPECT_TRUE(sharding0->HasSamePartitioning(*sharding0));
  {
    ...
    TF_ASSERT_OK_AND_ASSIGN(
        ShardingSpecRef sharding1,
        ShardingParamShardingSpec::Create(6, MemoryKind(), param1));
    EXPECT_TRUE(sharding0->HasSamePartitioning(*sharding1));
  }
  // Different number of shards.
  {
    ...
    TF_ASSERT_OK_AND_ASSIGN(
        ShardingSpecRef sharding1,
        ShardingParamShardingSpec::Create(3, MemoryKind(), param1));
    EXPECT_FALSE(sharding0->HasSamePartitioning(*sharding1));
  }
...
TEST_P(ShardingParamShardingSpecTest, WithMemoryKind) {
  ShardingParam param0{/*dim_shards=*/{2, 3},
                       {/*permutation=*/{1, 0}, /*axis_sizes=*/{3, 2}}};
  TF_ASSERT_OK_AND_ASSIGN(
      ShardingSpecRef sharding0,
      ShardingParamShardingSpec::Create(6, MemoryKind("pinned_host"), param0));
  {
    TF_ASSERT_OK_AND_ASSIGN(auto new_sharding,
                            sharding0->WithMemoryKind(MemoryKind("device")));
    ShardingParam param1{/*dim_shards=*/{2, 3},
    ...
    TF_ASSERT_OK_AND_ASSIGN(
        ShardingSpecRef sharding1,
        ShardingParamShardingSpec::Create(6, MemoryKind("device"), param1));
    EXPECT_EQ(*new_sharding, *sharding1);
  }
}
...
TEST_P(ShardingParamShardingSpecTest, Disassemble) {
...

  {
    ...
    for (int i = 0; i < 6; ++i) {
      ...
      EXPECT_EQ(*sharding, *SingleDeviceShardingSpec::Create(MemoryKind()));
    }
  }
}
